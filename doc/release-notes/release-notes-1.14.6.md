Moirai Core version 1.14.6 is now available from:

  <https://github.com/moirai-project/moirai/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Moirai Core users - miners, services,
...
Moirai network from 1 MOI to 0.01 MOI. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 MOI as
   change if possible.
2. If after signing there is more than 0.01 MOI left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 MOI, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 MOI will not be accepted as valid by default.
...
```
moirai-cli addnode <address> remove
```
...
```
moirai-cli liststucktransactions
```
...
```
moirai-cli setmaxconnections <desired_amount>
```
...
efforts to make Moirai more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Moirai Core.

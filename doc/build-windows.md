...
Below are some notes on how to build Moirai Core for Windows.
...
To build Moirai from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Moirai GitHub repository](https://github.com/moirai-project/moirai/)).
...
Note that for WSL v1 the Moirai Core source path MUST be somewhere in the default mount file system, for
example /usr/src/moirai, AND not under, for example, /mnt/d/moirai.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Moirai's dependencies):
...
way. This will install to `c:\workspace\moirai`, for example:

    make install DESTDIR=/mnt/c/workspace/moirai
